...
class base_c extends SGui {
	...
	private function getRights() {
		if (!isset($_COOKIE ['gid'])) $_COOKIE ['gid'] = 0;
		$gid = ( int ) $_COOKIE ['gid'];
		$cacheName = "action_code_group_" . $gid;
		$cache = SCache::getCacheEngine ( 'file' );
		$cache->init ( array ("dir" => SlightPHP::$appDir . "/cache", "depth" => 3 ) );
		...
		$this->params ['_userid'] = isset($_COOKIE ['admin_id']) ? $_COOKIE ['admin_id'] : '';
		$this->params ['_adminname'] = isset($_COOKIE ['admin_name']) ? $_COOKIE ['admin_name'] : '';
	}
	/**
	 * 获取uri参数
	 */
